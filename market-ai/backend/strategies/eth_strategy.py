...
class EthStrategy:
    ...
    def _is_pullback_toward(self, current_price: float, ema20: float, atr: float) -> bool:
        # ETH pullback within 1.5 ATR of EMA20
    ...
    def generate_signal(self, data: Dict[str, pd.DataFrame]) -> Dict[str, Any]:
        ...
        # Stronger volatility filtering than BTC
        if last15['candle_size'] > last15['atr14'] * 2.0:
            return self._build_no_trade("Excessive volatility (candle size > 2x ATR)")
        
        # Check Long conditions
        long_cond = {
            '4h_ema_cross': last4h['ema20'] > last4h['ema50'],
            '4h_trend': last4h['close'] > last4h['ema200'],
            '1h_adx': last1h['adx14'] > 23,
            '1h_rsi': last1h['rsi14'] > 52,
            '15m_pullback': self._is_pullback_toward(last15['low'], last15['ema20'], last15['atr14']),
            '15m_macd': last15['macd'] > 0,
            'vol_inc': last15['volume'] > prev15['volume'] and last15['volume'] > last15['vol_ma']
        }
        ...
        score_weights = {
            '4h_ema_cross': 15, '4h_trend': 20, '1h_adx': 15, '1h_rsi': 10,
            '15m_pullback': 15, '15m_macd': 10, 'vol_inc': 15
        }
        ...
        if (long_cond['4h_trend'] and short_cond['4h_ema_cross']) or (short_cond['4h_trend'] and long_cond['4h_ema_cross']):
            return self._build_no_trade("Conflicting 4H trend structure")
            
        direction = "NO_TRADE"
        score = 0
        reasons = []
        
        if long_cond['4h_trend'] and long_cond['4h_ema_cross'] and long_cond['15m_pullback']:
            if long_score >= 65:
                direction = "LONG"
                score = long_score
                reasons = [k for k, v in long_cond.items() if v]
        elif short_cond['4h_trend'] and short_cond['4h_ema_cross'] and short_cond['15m_pullback']:
            if short_score >= 65:
                direction = "SHORT"
                score = short_score
    ...
    def explain_signal(self, result: dict) -> str:
        ...
        explanation = f"ETH {result['direction']} Setup. Score: {result['signal_strength']}. "
